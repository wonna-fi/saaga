...
3. Script `detect-changes` compares the work tree against `docs/BASELINE`
...
5. If changes exist:
   ...
   - Script `generate-baseline` regenerates `docs/BASELINE`

### Validation Rules

- `dir` must exist and be a directory (defaults to current working directory if omitted)
- A `docs/BASELINE` file must exist (otherwise `detect-changes` throws; run `init` first)
...
| No BASELINE file exists | `detect-changes` throws `Error: BASELINE file not found... Run 'init' first` |
...
| # | Type | Details |
|---|------|---------|
| 1 | `script` | Name: `detect-changes`, args: `app_dir`, `output_dir`, sets `${changes}` in scope |
| 2 | `if` | Condition: `${changes.count} != 0` — gates all subsequent steps |
| 2.1 | `agent` | Prompt: `plan-update`, vars: `{app}`, `{changes_path}`, `{output_path}` (uses `${date}` in plan filename), `expect_file` asserts plan was written |
...
| 2.4 | `script` | Name: `generate-baseline`, regenerates `docs/BASELINE` |
...
| `src/scripts/generate-baseline.ts` | `generateBaseline()` | Regenerates `docs/BASELINE` after updates |
...
- **Depends on**: Agent backend, prompt templates, built-in scripts (`detect-changes`, `parse-plan`, `generate-baseline`), existing `docs/BASELINE`
