...
describe("buildProfile", () => {
  ...
  test("denies rule files and the machine-managed corpus files", () => {
    const profile = buildProfile({ appPath, docsDir, runDir });
    ...
    expect(profile.denyPaths).toContain(resolve(appPath, docsDir, "FORMAT"));
    // Generated by `generate-navigation` on every run: an agent edit here is
    // lost without a trace, and an agent "fixing" them churns every run.
    expect(profile.denyPaths).toContain(resolve(appPath, docsDir, "README.md"));
    expect(profile.denyPaths).toContain(resolve(appPath, docsDir, "GLOSSARY.md"));
  });

  test("does not deny the app's own README.md", () => {
    const profile = buildProfile({ appPath, docsDir, runDir });

    expect(profile.denyPaths).not.toContain(resolve(appPath, "README.md"));
  });
